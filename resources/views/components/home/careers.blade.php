...
<section class="py-16 bg-slate-950 text-white relative overflow-hidden"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="mt-4 md:mt-0 text-left md:text-right shrink-0">
                @if($totalActiveCount > 0)
                    <span class="inline-flex items-center gap-2 text-xs font-bold text-emerald-400 bg-emerald-950/80 px-3.5 py-1.5 rounded-full border border-emerald-800/50 shadow-lg shadow-emerald-950/50">
                        ...
                        {{ $totalActiveCount }} Active Job Vacancies Available Today
                    </span>
                @else
        ...
        {{-- 1. Official Government Partner Agencies Strip --}}
        <div class="mb-12">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
                @foreach($partnerAgencies as $agency)
                    <button type="button"
                            @click="selectedAgency = {{ json_encode($agency) }}; agencyModalOpen = true"
                        ...
                        <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white p-1.5 shadow-md flex items-center justify-center mb-3 group-hover:scale-105 group-hover:shadow-amber-400/20 transition-all">
                            <img src="{{ $agency['logo'] }}" 
                                 ...
                                 loading="lazy"
                                 onerror="this.onerror=null; this.src='{{ asset('img/camsur-logo.png') }}';">
                        </div>
                        <div class="w-full">
                            ...
                            <p class="text-[10px] text-slate-400 line-clamp-1 mt-0.5 font-medium" title="{{ $agency['fullName'] }}">
                                {{ $agency['fullName'] }}
                            </p>
                            <span class="inline-block mt-2 text-[9px] font-semibold px-2 py-0.5 rounded-full border {{ $agency['badgeColor'] }}">
                                {{ $agency['tag'] }}
                            </span>
        ...
        {{-- 2. Featured Upcoming Job Fairs, SRA & Employment Highlights (Across ALL Categories) --}}
        <div class="mb-12 bg-slate-900/90 border border-slate-800/90 rounded-3xl p-5 sm:p-7 shadow-2xl backdrop-blur-xl">
            
            {{-- Panel Header & Event Selector Tabs --}}
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-slate-800">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($careerEvents as $idx => $event)
                        <button type="button"
                                @click="activeEventIndex = {{ $idx }}"
                                :class="activeEventIndex === {{ $idx }} 
                            ...
                            <span>
                                @if($event['id'] === 'job-fair') 🎪
                                @elseif($event['id'] === 'overseas-sra') ✈️
                                @elseif($event['id'] === 'student-jobs') 🎓
                                @elseif($event['id'] === 'skills-seminar') 🛠️
                                @elseif($event['id'] === 'csc-plantilla') 🏛️
                                @endif
                            </span>
            ...
            {{-- Dynamic Event Content Display --}}
            @foreach($careerEvents as $idx => $event)
                <div x-show="activeEventIndex === {{ $idx }}" 
                    ...
                    <div class="lg:col-span-5 relative rounded-2xl overflow-hidden border border-slate-700/70 aspect-video lg:aspect-4/3 bg-slate-950 group">
                        ...
                        {{-- Top Badges: Event Type & CMS Sourcing Method --}}
                        <div class="absolute top-3 left-3 right-3 flex items-center justify-between gap-2 flex-wrap">
                            <span class="text-[11px] font-bold px-2.5 py-1 rounded-full border backdrop-blur-md {{ $event['badgeStyle'] }}">
                                {{ $event['badge'] }}
                            </span>
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-md bg-black/75 text-slate-300 border border-white/10 backdrop-blur-md flex items-center gap-1">
                                <span>{{ $event['cmsIcon'] }}</span>
                                <span>{{ $event['cmsMethod'] }}</span>
                            </span>
                        </div>

                        {{-- Bottom Event Meta: Schedule & Venue --}}
                    ...
                    {{-- Event Text Details & Key Highlights --}}
        ...
        {{-- 3. Four Core Career Portals: Compact Focused 3-Card Carousel with Auto-Loop --}}
        <div class="mb-4"
             @mouseenter="stopPortalLoop()"
             @mouseleave="startPortalLoop()">
            
            {{-- Carousel Section Header with Controls --}}
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                {{-- Carousel Navigation Arrows & Active Counter --}}
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-2 mb-4 scrollbar-none">
                @foreach($portalCards as $card)
                    ...
                    <button type="button"
                        ...
                        <span :class="activePortalIdx === {{ $i }} ? 'bg-slate-950/20 text-slate-950' : 'bg-slate-800 text-slate-400'"
            ...
            {{-- Responsive Interactive Carousel Track: 1 Card per View (Landscape Layout) --}}
            <div class="relative py-1">
                @foreach($portalCards as $card)
                    @php $i = $card['index']; @endphp
                    <div x-show="activePortalIdx === {{ $i }}"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center relative z-10">
                            ...
                            {{-- Left Side: Landscape Media & Category Identity --}}
                            <div class="lg:col-span-5 flex flex-col justify-between h-full">
                                <div class="relative rounded-2xl overflow-hidden border border-slate-700/70 aspect-video bg-slate-950 shadow-inner group">
                                    <img src="{{ $card['bannerImage'] }}" 
                                         alt="{{ $card['title'] }}" 
                                         class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                                         loading="lazy">
                                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent"></div>

                                    {{-- Active Focus Pill (Cleanly positioned inside media overlay with no overlap) --}}
                                    ...
                                    {{-- Vacancies Counter Badge --}}
                                    <div class="absolute top-3 right-3">
                                        <span class="text-[11px] font-bold px-3 py-1 rounded-full border backdrop-blur-md {{ $card['badgeColor'] }} flex items-center gap-1.5 shadow-md">
                                            @if($card['vacanciesActive'])
                                                <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                                            @else
                                                <span class="w-2 h-2 rounded-full bg-slate-500"></span>
                                            @endif
                                            <span>{{ $card['vacanciesLabel'] }}</span>
                                        </span>
                                    </div>

                                    {{-- Bottom Overlay Info in Media Box --}}
                                    <div class="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p class="text-[10px] font-bold text-slate-300 uppercase tracking-wider">Gateway {{ $i + 1 }} of 4</p>
                                                <p class="text-xs font-bold text-white">{{ $card['title'] }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {{-- Right Side: Expanded Landscape Info & Action --}}
                            <div class="lg:col-span-7 flex flex-col justify-between space-y-5">
                                ...
                                <div class="pt-4 border-t border-slate-800/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                                    ...
                                    <a href="{{ $card['route'] }}"
                                       class="inline-flex items-center justify-center gap-2 font-bold py-3 px-6 rounded-xl transition-all duration-200 text-xs shadow-lg {{ $card['btnClass'] }} group/btn shrink-0">
                ...
                {{-- Carousel Pagination Dots (4 Dots for 4 Categories) --}}
                <div class="flex items-center justify-center gap-2 mt-6">
                    @foreach($portalCards as $card)
                        @php $i = $card['index']; @endphp
                        <button type="button"
                                @click="setPortal({{ $i }})"
                                :class="activePortalIdx === {{ $i }} ? 'w-8 bg-amber-400' : 'w-2 bg-slate-700 hover:bg-slate-600'"
    ...
    {{-- 🏛️ Interactive Accredited Partner Agency Mandate & Contribution Modal --}}
    <div x-show="agencyModalOpen"
        ...
        <div @click.away="agencyModalOpen = false"
            ...
            <div class="p-6 sm:p-8 overflow-y-auto space-y-6 text-sm text-slate-300 leading-relaxed">
                ...
                {{-- 1. Why Is This Agency Here? (Core Contribution to the Portal) --}}
                ...
                {{-- 3. Key Services Facilitated on this Portal --}}
                <div>
